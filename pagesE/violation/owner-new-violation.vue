	...
	<view class="new-violation-container">
		...
		<view class="custom-navbar">
			<view class="navbar-content" :style="{ marginTop: statusBarHeight + 'px' }">
				...
				<!-- 中间标题 -->
				<view class="navbar-title">违规查询</view>
				<!-- 右侧按钮区域 -->
				<view class="navbar-right">
					<!-- 显示当前业主信息（仅显示，不可点击） -->
					<view class="owner-display" v-if="currentUserRole === 'owner' && currentUserPhone">
						<text class="owner-icon">👤</text>
						<text class="owner-text">{{ formatPhoneNumber(currentUserPhone) }}</text>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="filter-control-panel">
			<view class="filter-options-horizontal">
				...
				<view class="filter-item-horizontal" @click="showStatusModal = true">
					<view class="filter-content">
						<text class="filter-label">进出状态</text>
						<view class="filter-value">
		...
		<view class="high-risk-vehicle-standalone" v-if="mostViolationCar.count > 0">
			...
			<view class="warning-content">
				<text class="warning-title">高频提醒</text>
				...
				<view class="info-content">
					...
					<text class="info-desc">
						...
						<template v-if="selectedFilter !== 'all'">
							进出状态：{{ getStatusText(selectedFilter) }}
						</template>
					</text>
				</view>
			...
			<view class="records-list" v-if="filteredRecords.length > 0">
				<view v-for="(record, index) in filteredRecords" :key="index" class="record-item">
					...
					<view class="record-header">
						<view class="header-left">
							...
							<view class="status-badge" :class="'status-' + record.status">
								<u-icon v-if="record.status === 'entered'" name="play-circle" size="18"
									color="#ffffff"></u-icon>
								<u-icon v-else-if="record.status === 'left'" name="pause-circle" size="18"
									color="#ffffff"></u-icon>
					...
					<view class="record-main-content">
						<!-- 时间信息网格 -->
						<view class="time-info-grid" :class="{ 'no-leave-time': !record.leaveTime }">
							<!-- 预约时间-->
							...
							<!-- 停车时长和离场时间-->
							<view class="time-block leave-block" v-if="record.leaveTime">
								<view class="time-label">
									<text class="time-icon">🚀</text>
									<text>离场时间</text>
								</view>
							...
							<!-- 停车时长计算 -->
							<view class="time-block duration-block" v-if="record.leaveTime">
								<view class="time-label">
		...
		<!-- 状态选择弹窗 -->
		<u-modal :show="showStatusModal" title="选择进出状态" :show-cancel-button="true" :show-confirm-button="false"
			...
			<view class="status-modal">
				<view v-for="(status, index) in statusOptions" :key="index" class="status-modal-item"
					...
					<view class="status-icon" :class="'status-' + status.value">
						<u-icon v-if="status.value === 'entered'" name="play-circle" size="24"
							color="#4caf50"></u-icon>
						<u-icon v-else-if="status.value === 'left'" name="pause-circle" size="24"
							color="#2196f3"></u-icon>
...
export default {
	...
	computed: {
		...
		statusOptions() {
			...
			},
			{
				label: '已进场',
				value: 'entered',
				count: statusStats.entered
			},
			{
				label: '已离场',
				value: 'left',
				count: statusStats.left
			}
			];
		},
		...
		dynamicVehicleList() {
			...
			filteredRecords.forEach(record => {
				...
				const vehicleMonthlyRecords = vehicleTotalRecords.filter(r => {
					const dateStr = this.formatDateToString(r.createdAt);
				...
				// 更新最后违规日期
				const currentDateStr = this.formatDateToString(record.createdAt);
				if (currentDateStr && (!vehicleStats[plateNumber].lastViolation || currentDateStr > vehicleStats[
					plateNumber].lastViolation)) {
					vehicleStats[plateNumber].lastViolation = currentDateStr;
				}
	...
	async mounted() {
		...
		// 等待用户手机号获取完成后，初始化数据（从violations表获取并按手机号筛选）
		await this.initializeViolationData();
		
		// 设置月违规提示
		this.setMonthlyViolationAlert();
		// 从ownerinfo表加载信用分
		await this.loadCreditScoreFromOwnerInfo();
		// 初始化面包屑
	...
	methods: {
		...
		formatDateToString(dateValue) {
			if (!dateValue) {
				console.warn('⚠️ 日期值为空:', dateValue);
				return null;
			}
		...
		getCurrentUserPhone() {
			try {
				...
				console.log('📱 获取用户手机号:', this.currentUserPhone);
			} catch (error) {
		...
		// 根据手机号筛选违规记录的通用方法
		filterRecordsByOwnerPhone(records) {
			if (!this.currentUserPhone || !Array.isArray(records)) {
				return records || [];
			}

			const filtered = records.filter(record => {
				// 如果记录有ownerPhone字段，必须与当前用户手机号匹配
				if (record.ownerPhone) {
					const match = record.ownerPhone === this.currentUserPhone;
					if (!match) {
						console.log('🚫 筛选掉不匹配的记录:', {
							recordOwnerPhone: record.ownerPhone,
		...
		// 从ownerinfo表加载信用分
		async loadCreditScoreFromOwnerInfo() {
			try {
				if (!this.currentUserPhone) {
					console.warn('⚠️ 无法获取信用分：当前用户手机号为空');
					this.creditScore = 85; // 默认信用分
					return;
				}

				console.log('📊 开始从ownerinfo表查询信用分:', this.currentUserPhone);
				
				// 调用API查询业主信息（包含信用分）
				const response = await request({
					url: `/parking/ownerinfo/listByPhone?userphone=${this.currentUserPhone}`,
					method: 'GET',
					timeout: 10000
				});

				console.log('📊 ownerinfo表查询响应:', response);

				// 解析响应数据
				if (response.data && response.data.data && Array.isArray(response.data.data) && response.data.data.length > 0) {
					const ownerInfo = response.data.data[0]; // 取第一个匹配的业主记录
                    console.log('ownerInfo',ownerInfo)
					const creditScore = ownerInfo.credit_score || ownerInfo.creditScore || 0;
					
					if (creditScore > 0) {
						this.creditScore = creditScore;
						console.log('✅ 从ownerinfo表获取信用分成功:', creditScore);
					} else {
						// 如果数据库中信用分为0或空，使用默认值
						this.creditScore = 85;
						console.log('⚠️ 数据库中信用分为空或0，使用默认值:', this.creditScore);
					}
				} else {
					// 如果查询不到业主信息，使用默认信用分
					this.creditScore = 85;
					console.log('⚠️ 未查询到业主信息，使用默认信用分:', this.creditScore);
				}
			} catch (error) {
				console.error('❌ 从ownerinfo表查询信用分失败:', error);
				// 查询失败时使用默认信用分
				this.creditScore = 85;
				console.log('⚠️ 查询失败，使用默认信用分:', this.creditScore);
			}
		},

		// 从violations表初始化违规数据（按当前业主手机号筛选）
		async initializeViolationData() {
			if (this.currentUserPhone) {
				console.log('🔍 根据手机号查询真实违规数据:', this.currentUserPhone);
				...
				try {
					// 调用后端API获取违规记录，根据手机号筛选
					const response = await violationApi.getViolations({
						phone: this.currentUserPhone, // 传入手机号作为查询条件
						page: 1,
						size: 1000 // 获取足够多的记录用于分析
					});
					
					console.log('✅ 违规数据获取成功:', response);
					...
					if (response && Array.isArray(response)) {
						...
						rawRecords = [];
									...
									// 转换数据格式，确保字段名一致
				let convertedRecords = rawRecords.map(record => {
					const leaveTime = record.leaveTime || record.leave_time || '';
					// 根据是否有离场时间决定状态
					let newStatus = 'entered'; // 默认已进场
					let newStatusText = '已进场';
					if (leaveTime) {
						newStatus = 'left';
						newStatusText = '已离场';
					}
					...
					return {
						id: record.id || record.violationId || record.record_id,
						plateNumber: record.plateNumber || record.plate_number || record.carNumber || record.car_no,
						createdAt: record.createdAt || record.created_at || record.violationTime || record.violation_time,
						reason: record.reason || record.violationType || record.violation_type || record.violationReason,
						status: newStatus, // 使用新的状态逻辑
						statusText: newStatusText, // 使用新的状态文本
						location: record.location || record.violationLocation || record.violation_location || '',
						severity: record.severity || 'moderate',
						enterTime: record.enterTime || record.enter_time || record.createdAt,
						leaveTime: leaveTime,
						appointmentTime: record.appointmentTime || record.appointment_time || '',
						description: record.description || record.remark || '',
						reporterId: record.reporterId || record.reporter_id || '',
						handlerId: record.handlerId || record.handler_id || '',
						// 添加业主手机号字段映射
						ownerPhone: record.ownerPhone || record.owner_phone || record.userPhone || record.user_phone || record.phone || '',
						// 保留原始数据以备调试
						_originalData: record
					};
				});

				// 根据当前用户手机号筛选数据，只显示属于当前用户的违规记录
				this.violationRecords = this.filterRecordsByOwnerPhone(convertedRecords);
					
					console.log('📊 处理后的违规记录数量:', this.violationRecords.length);
					
					// 基于真实数据更新车辆列表
					this.updateVehicleListFromRecordsWithPhoneFilter();
					
				} catch (error) {
					console.error('❌ 获取违规数据失败:', error);
					uni.showToast({
						title: '违规数据加载失败',
						icon: 'none',
						duration: 2000
				...
				console.warn('⚠️ 未获取到用户手机号，无法筛选违规数据');
				// 如果没有手机号，初始化为空数据
				this.violationRecords = [];
				this.vehicleList = [];
			}
		...
		// 根据手机号筛选初始化数据
		initializeDataWithPhoneFilter() {
			// 从违规记录中提取所有唯一的车辆，并根据手机号筛选
			this.updateVehicleListFromRecordsWithPhoneFilter();
			// 计算最多违规车辆
			this.calculateMostViolationCar();
		},
		...
		// 根据手机号从违规记录中更新车辆列表
		updateVehicleListFromRecordsWithPhoneFilter() {
			// API已经根据手机号进行了筛选，所以这里直接使用违规记录
			const userViolationRecords = this.violationRecords;
			
			console.log('📊 处理违规记录数量:', userViolationRecords.length);
			
			const vehicleMap = {};
			const currentMonth = new Date().getMonth() + 1;
			const currentYear = new Date().getFullYear();

			// 遍历筛选后的违规记录，统计每个车辆的情况
			userViolationRecords.forEach(record => {
				const plateNumber = record.plateNumber;
				...
				// 统计当月违规次数
				const dateStr = this.formatDateToString(record.createdAt);
				if (dateStr) {
					const [year, month] = dateStr.split('-').map(Number);
					...
					// 更新最后违规日期
					if (!vehicleMap[plateNumber].lastViolation || dateStr > vehicleMap[plateNumber].lastViolation) {
						vehicleMap[plateNumber].lastViolation = dateStr;
					}
				}
				...
				vehicleMap[plateNumber].reasonCount[reason] = (vehicleMap[plateNumber].reasonCount[reason] || 0) + 1;
			});
			...
			// 同时更新违规记录为筛选后的记录
			this.violationRecords = userViolationRecords;
		...
		// 注意：已移除生成模拟数据的方法，现在从后端API获取真实数据
		// generateCurrentMonthData() - 已删除，不再生成模拟数据

		// 根据状态代码获取状态文本
		getStatusTextByCode(statusCode) {
			const statusMap = {
				'entered': '已进场',
				'left': '已离场',
				'pending': '待处理',
		...
		generateDayData() {
			...
			for (let i = 0; i < 7; i++) {
				...
				let filteredRecords = this.violationRecords.filter(record => {
					const recordDate = this.formatDateToString(record.createdAt);
		...
		getComparisonTableData() {
			...
			return this.comparisonVehicles.map(plateNumber => {
				...
				const monthlyCount = this.violationRecords.filter(record => {
					const parsed = this.parseCreatedAt(record.createdAt);
					if (!parsed) return false;
					
					const [year, month] = parsed.dateStr.split('-').map(Number);
...
${ ranking <= 3 ? '⚠️ 立即整改停车行为，避免进一步处罚！' : '继续保持良好的停车习惯'}`,
	...
	generateSuggestions() {
		...
		if (violationTypes.length > 0) {
			...
			'未经授权停车': {
				icon: '🔒',			
					text: `请确保在预约时间内停车，避免无预约停车行为(已发${mostCommon.count}次。`,
						action: '了解预约规则',
							actionType: 'rules',
								actionData: {
					type: '预约规则'
...
calculateMostViolationCar() {
	...
	this.violationRecords.forEach(violation => {
		const parsed = this.parseCreatedAt(violation.createdAt);
		if (!parsed) return;
...
parseCreatedAt(createdAt) {
	try {
		...
		if (typeof createdAt === 'string') {
			...
			dateStr = parts[0]; // 取日期部分
		} else {
			// 如果不是字符串，转换为 Date 对象再格式化
		...
		return {
			dateStr,
			fullStr: typeof createdAt === 'string' ? createdAt : new Date(createdAt).toISOString().replace('T', ' ').slice(0, 19)
		};
	} catch (error) {
		console.warn('日期解析失败:', createdAt, error);
		return null;
	}
},
		...
		title: `🔔 针对"${data.type}"设置提醒（已发生${data.count}次）`,
							...
							success: (res) => {
								if (res.confirm) {
									...
									if (step <= 4) {
										...
										uni.showModal({
											title: emoji + ' 练习完成',
											content: '🎉 恭喜完成停车练习！\n\n📊 最终得分：' + score + '/100分\n🏅 评价等级：' + level + '\n\n📈 能力分析：\n环境观察：' + steps[0].title + ' ✓\n角度控制：' + steps[1].title + ' ✓\n倒车技巧：' + steps[2].title + ' ✓\n精准停车：' + steps[3].title + ' ✓\n\n' + (score >= 80 ? '🎖您已具备熟练的停车技能！' : '💪 继续练习，提升停车技巧！'),
											showCancel: true,
											cancelText: '再练一次',
											confirmText: '查看进度',
											success: (resultRes) => {
			...
			showLearningProgress() {
				try {
					...
					uni.showModal({
						title: '📊 学习进度',
						content: '🎓 您的停车技能进度：\n\n🏆 当前等级：' + progress.skillLevel + '\n🎯 练习次数：' + progress.totalPractices + '次\n📊 最佳成绩：' + progress.bestScore + '分\n📹 观看视频：' + progress.completedVideos + '个\n\n📈 升级条件：\n新手→良好：练习3次，成绩70+\n良好→熟练：练习5次，成绩80+\n熟练→专家：练习10次，成绩90+\n\n💪 继续努力，提升停车技能！',
						showCancel: false,
						confirmText: '继续学习'
					});
				} catch (e) {
			...
			reserveToday() {
				...
				for (let hour = Math.max(currentHour + 1, 8); hour <= 22; hour++) {
					timeSlots.push(hour.toString().padStart(2, '0') + ':00-' + (hour + 2).toString().padStart(2, '0') + ':00');
				}
