		...
		<view class="form-container">
			...
			<view class="form-item">
				...
				<view class="owner-phone-container">
					<input 
						class="input owner-phone-input" 
						v-model="form.ownerPhone" 
						placeholder="请输入要访问的业主手机号" 
						maxlength="11"
						type="number"
						@input="onOwnerPhoneInput"
						@blur="onOwnerPhoneBlur"
					/>
				...
				<view class="action-buttons" style="margin-top: 20rpx;">
					...
					<button 
						...
						class="verify-btn" 
						@click="manualValidateOwnerPhone"
						:disabled="!canManualVerify"
						size="mini"
						type="primary"
						style="font-size: 26rpx; padding: 12rpx 24rpx; margin-right: 20rpx;"
					>
						{{ getVerifyButtonText() }}
					</button>
					...
					<!-- 验证成功后显示重新验证按钮 -->
					<button 
						v-if="ownerVerifyStatus === 'valid'"
						class="re-verify-btn" 
						@click="reVerifyOwner"
						size="mini"
						type="default"
						style="font-size: 24rpx; padding: 10rpx 20rpx; margin-right: 20rpx;"
					>
						🔄 重新验证
					</button>
          ...
          <button 
            class="check-data-btn" 
            @click="checkDatabaseData"
            size="mini"
            type="default"
            style="margin-left: 10rpx; font-size: 24rpx;"
          >
            查看数据
          </button> -->
				</view>
...
本隐私政策最后更新时间：2025年6月26日`
		...
		computed: {
			canSubmit() {
				...
				return hasNickname && hasReason && hasPhone && hasOwnerPhone && hasValidOwner && hasAddress && hasAgreed && !this.submitting;
			},
...
✅ 信息将自动填入，请确认并完善其他必要信息。`,
					...
					// 检查是否有其他格式的二维码数据
					if (options && (options.data || options.butlerPhone || options.type === 'butler_invitation' || options.bp || options.t === 'bi')) {
						let qrData = {};
						...
						// 检查是否是管家邀请的访客申请
						if (qrData.butlerName || qrData.butlerPhone || qrData.fullAddress || qrData.type === 'butler_invitation') {
...
地址信息将自动填入，管家信息也将自动设置。`;
							...
							if (addressInfo.community || addressInfo.province) {
								// 直接设置地址信息到表单
								this.form.addressInfo = { ...addressInfo };
			...
			async checkOwnerMonthlyTicket(phone) {
				...
				try {
					...
					if (data && data.code === "0" && data.data && data.data.code === "0") {
						...
						const nameFields = ['userName', 'ownername', 'username', 'name', 'ownerName', 'realName']; // userName放在首位
						
						if (ownerInfo) {
							for (const field of nameFields) {
								if (ownerInfo[field] && typeof ownerInfo[field] === 'string' && ownerInfo[field].trim()) {
						...
						if (ownerInfo) {
							...
							// 尝试从分段地址信息构建
							else if (ownerInfo.building || ownerInfo.units || ownerInfo.floor || ownerInfo.roomnumber) {
								const addressParts = [];
